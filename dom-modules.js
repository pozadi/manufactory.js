...
(function() {
  var BaseModule, ModuleInfo, action, buildModule, genName, jqueryPlugins, lastNameId, modulesAPI, __moduleInstances, __modules,
    __slice = [].slice,
  ...
  BaseModule = (function() {
    ...
    BaseModule.prototype._bind = function() {
      var elementName, eventMeta, eventName, handler, moduleInstance, selector, _i, _len, _ref, _results,
        _this = this;
      moduleInstance = this;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        selector = this.constructor.ELEMENTS[elementName].selector;
        if (typeof handler === 'string') {
          handler = this[handler];
        }
        _results.push((function(eventName, selector, handler) {
          return _this.root.on(eventName, selector, function() {
            var args;
            args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            args.unshift(this);
            return handler.apply(moduleInstance, args);
          });
    ...
    BaseModule._bind = function() {
      var elementName, eventMeta, eventName, handler, moduleClass, selector, _i, _len, _ref, _results,
        _this = this;
      moduleClass = this;
      _ref = this.EVENTS;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        eventMeta = _ref[_i];
        handler = eventMeta.handler, eventName = eventMeta.eventName, elementName = eventMeta.elementName;
        selector = this.ELEMENTS[elementName].selector;
        if (typeof handler === 'string') {
          handler = this.prototype[handler];
        }
        _results.push((function(eventName, selector, handler) {
          return $(document).on(eventName, "" + _this.ROOT_SELECTOR + " " + selector, function() {
